...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // --- Step 2: Parallel data collection + scoring ---
    // Run EVERYTHING in parallel to stay within 60s
    log.push(`[${elapsed()}] Starting parallel scoring & data collection...`);
    ...
    const [
      ...
      // Score all CORE-EEAT dimensions in parallel
      ...coreEeatDims.map((dim) =>
        scoreOneDimension(html, url, dim, "full", quality).catch((e) => {
          log.push(`[${elapsed()}] CORE-EEAT ${dim} failed: ${e.message}`);
          return [] as Omit<AuditItem, "id" | "audit_id">[];
        })
      ),
      // Score all CITE dimensions in parallel
      ...citeDims.map((dim) =>
        scoreOneCiteDimension(html, url, dim, "full", quality).catch((e) => {
          log.push(`[${elapsed()}] CITE ${dim} failed: ${e.message}`);
          return [] as Omit<AuditItem, "id" | "audit_id">[];
        })
      ),
    ]);

    log.push(`[${elapsed()}] All parallel tasks completed`);
    ...
    if (seoResult.status === "rejected") log.push(`[${elapsed()}] SEO failed: ${seoResult.reason?.message}`);
    if (geoResult.status === "rejected") log.push(`[${elapsed()}] GEO failed: ${geoResult.reason?.message}`);

    // Collect scored items
    const allScoredItems: Omit<AuditItem, "id" | "audit_id">[] = [];
    const coreEeatCount = coreEeatDims.length;
